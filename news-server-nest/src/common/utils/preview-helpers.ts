 ...
 * 메시지 텍스트에서 기사 정보 추출
 */
export async function extractArticlePreview(
  pool: Pool | PoolConnection,
  content: string,
): Promise<any | null> {
  ...
  const article = await getArticleByUrl(pool, firstUrl);
 ...
 * 형식: /debate/{topicId}, /topics/{topicId}
 */
  ...
  try {
    ...
    if (topicRows.length > 0) {
      ...
      if (topic.vote_end_at) {
        ...
        topic.vote_remaining_time = null;
      }
 ...
 * 메시지 텍스트에서 토픽 정보 추출
 */
export async function extractTopicPreview(
  pool: Pool | PoolConnection,
  content: string,
): Promise<any | null> {
  ...
  const topic = await getTopicById(pool, topicId);
  return topic;
}
